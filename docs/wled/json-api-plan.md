...
| 26 Effect parameters | ◐ `c1`–`c3` and `o1`–`o3` stored on `Segment` (`custom`, `checks`; defaults 128/128/16 and off), parsed with the level and switch rules, applied after `pal` with `c3` confined to 0–31, reported in the state document, counted as effect changes, and handed to effects in `Params`. `smart-leds-fx` effects will take named settings that each registry entry maps these onto. `fxdef` and `/json/fxdata` pending |
