...
| 15 Catalogues | ✅ `EffectKind` ids follow the reference numbering (Solid 0, Rainbow 9, and 66 `smart-leds-fx` effects under the reference ids and names of their closest counterparts) with descriptors; `Descriptor` parser (sliders, colours, palette, flags, defaults after the last `;`); palettes 0–12 under reference ids and names (Default = segment colours, Random Cycle, the four built from segment colours, the seven classic tables with Party, Rainbow and Rainbow Bands gamma-compensated), shown on every in-order stepped effect and Rainbow; `luxa_segment::CATALOGUE` feeds the engine |
...
| 26 Effect parameters | ✅ `c1`–`c3` and `o1`–`o3` stored on `Segment` (`custom`, `checks`; defaults 128/128/16 and off), parsed with the level and switch rules, applied after `pal` with `c3` confined to 0–31, reported in the state document, counted as effect changes, and handed to effects in `Params`. Each registry entry's `Controls` map speed, intensity, `c1`–`c3` and `o1`–`o3` onto the named settings its `smart-leds-fx` effect reads (a test checks every shown control against `Effect::settings`); an effect whose speed drives a setting draws every frame. 16 effects use it, from Bpm and Sine to Railway and Flow. `fxdef` loads the defaults a descriptor names when it selects a different effect — sliders and checkboxes it does not name return to their defaults, `pal`/`rev`/`mi` change only when named, later keys in the request still apply, `m12`/`si`/`rY`/`mY` are not modelled — from a `Catalogue` table `luxa-segment` builds from the descriptors at compile time. `/json/fxdata` writes each effect's descriptor after `@` (`""` for none and for gaps), streamed like every document |
| 36 Transition styles and blend modes | ◐ `bm` stored exactly as sent (0–255) and reported back, parsed with the level rules and applied after `o1`–`o3`, counted as an options change; `luxa_msg::BlendMode` names the 17 modes, and a number with none draws on top. The compositor blends each segment's pixels onto what the segments beneath drew — top, bottom, add (scaled back to keep the hue), subtract, difference, average, multiply, divide, lighten, darken, screen, overlay, hard light, soft light, dodge, burn, stencil — then mixes the result in by opacity, so a translucent segment now shows what lies beneath instead of darkening. Integer rounding is Luxa's own. An effect whose registry entry names an Overlay checkbox (Two Dots, Lightning, Spots, Spots Fade, Glitter) draws only its highlights while that checkbox is ticked, and the compositor blends it with stencil unless the segment names a blend mode of its own. Transition styles (`bs`) pending |
| 29 Transitions | ◐ `tt` and `transition` reach the renderer as `State::change_transition` (one batch is one change); `luxa-output::BrightnessFade` fades brightness and power linearly, restarting from what is shown on a new target; the compositor fades each segment's colours, opacity and on/off the same way, fading out before an off segment stops drawing. Palettes fade entry by entry into the next palette (switching a palette on or off is immediate), and Random Cycle fades into a new palette every 5 s. Effect crossfades and transition styles (`bs`) pending |
