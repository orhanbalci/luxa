...
| `fx`: `/json/eff` lists 60 effects under reference ids and names, `RSVD` gaps between | Solid and Rainbow natively, 58 from `smart-leds-fx` (41 of WS2812FX lineage and 17 written from the reference's descriptors; none has the reference's exact look); unknown ids fall back to Solid |
